...
exports.RoomActor = void 0;
exports.createInitialRoomState = createInitialRoomState;
const uuid_1 = require("uuid");
const types_1 = require("../types");
const auction_engine_1 = require("./auction-engine");
...
// ============================================================
class RoomActor {
    constructor(io, initialState, hostToken) {
        this.queue = [];
    ...
    async save() {
        try {
            await (0, db_1.saveRoomSnapshot)(this.state.roomCode, this.state.hostSessionId, this.hostToken, this.state);
        }
        catch (err) {
    ...
    startTimer() {
        ...
        this.timerInterval = setInterval(() => {
            this.enqueue(async () => {
                ...
                const finalizeUpdates = (0, auction_engine_1.finalizePlayer)(this.state, player, soldTo, soldPrice);
                const logMsg = soldTo
                    ? `🔨 SOLD! ${player.name} → ${soldTo} for ${(0, auction_engine_1.formatLakhs)(soldPrice)}`
                ...
                const advance = (0, auction_engine_1.advanceToNextPlayer)(this.state, this.state.allPlayers);
                this.state = { ...this.state, ...advance };
                if (advance.status === "ended") {
                    this.log("host_action", "🏆 Auction has ended! All sets complete.");
                    this.stopTimer();
                    try {
                        await (0, db_1.saveRoomSummary)(this.state.roomCode, this.state);
                    }
                    catch { }
                }
                else {
    ...
    // ============================================================
    handleConnect(socket, sessionId) {
        ...
        const ownedTeamCode = Object.values(this.state.teams).find((t) => t.ownerSessionId === sessionId)?.code;
        if (ownedTeamCode) {
    ...
    handleDisconnect(sessionId) {
        this.enqueue(async () => {
            const team = Object.values(this.state.teams).find((t) => t.ownerSessionId === sessionId);
            if (team) {
                this.state = {
                    ...this.state,
                    teams: {
    ...
    // Claim team
    // ============================================================
    handleClaimTeam(socket, sessionId, displayName, teamCode) {
        this.enqueue(async () => {
    ...
    // Bid
    // ============================================================
    handleBid(socket, sessionId, teamCode, bidAmount) {
        this.enqueue(async () => {
            ...
            const error = (0, auction_engine_1.validateBid)(this.state, teamCode, bidAmount, types_1.DEFAULT_BID_SLABS);
            if (error) {
            ...
            this.log("bid", `${team.name} bid ${(0, auction_engine_1.formatLakhs)(bidAmount)}`, teamCode);
            await this.save();
        });
    }
    // ============================================================
    ...
    handleStartAuction(socket, hostToken) {
        if (!this.verifyHost(socket, hostToken))
            return;
        this.enqueue(async () => {
            ...
            const setPlayers = this.state.allPlayers.filter((p) => p.set === firstSet && p.status === "upcoming");
    ...
    handlePauseAuction(socket, hostToken) {
        if (!this.verifyHost(socket, hostToken))
            return;
        this.enqueue(async () => {
    ...
    handleResumeAuction(socket, hostToken) {
        if (!this.verifyHost(socket, hostToken))
            return;
        this.enqueue(async () => {
    ...
    handleSkipPlayer(socket, hostToken) {
        if (!this.verifyHost(socket, hostToken))
            return;
        this.enqueue(async () => {
            ...
            const finalizeUpdates = (0, auction_engine_1.finalizePlayer)(this.state, player, null, null);
            this.state = { ...this.state, ...finalizeUpdates };
    ...
    handleExtendTimer(socket, hostToken, extraSeconds) {
        if (!this.verifyHost(socket, hostToken))
            return;
        this.enqueue(async () => {
            if (this.state.status !== "live" || !this.state.timerEndsAt)
                return;
            this.state = {
                ...this.state,
    ...
    handleSetTimerDuration(socket, hostToken, seconds) {
        if (!this.verifyHost(socket, hostToken))
            return;
        this.enqueue(async () => {
    ...
    handleKickUser(socket, hostToken, targetSessionId) {
        if (!this.verifyHost(socket, hostToken))
            return;
        this.enqueue(async () => {
            // Find team owned by this session
            const team = Object.values(this.state.teams).find((t) => t.ownerSessionId === targetSessionId);
            if (team) {
                this.state = {
                    ...this.state,
                    teams: {
                ...
                this.log("kicked", `🚫 ${team.ownerName} kicked — ${team.name} now unclaimed`, team.code);
            }
    ...
    handleReassignTeam(socket, hostToken, teamCode, newSessionId, newOwnerName) {
        if (!this.verifyHost(socket, hostToken))
            return;
        this.enqueue(async () => {
            const team = this.state.teams[teamCode];
            ...
            this.log("team_reassigned", `🔄 ${team.name} reassigned to ${newOwnerName}`, teamCode);
            await this.save();
        });
    }
    handleEditPurse(socket, hostToken, teamCode, newPurse) {
        if (!this.verifyHost(socket, hostToken))
            return;
        this.enqueue(async () => {
            const team = this.state.teams[teamCode];
            ...
            this.log("host_action", `💰 ${team.name} purse edited to ${(0, auction_engine_1.formatLakhs)(newPurse)}`, teamCode);
            await this.save();
        });
    }
    // ============================================================
...
exports.RoomActor = RoomActor;
// ============================================================
// Factory: create initial RoomState from parsed players
// ============================================================
function createInitialRoomState(roomCode, hostSessionId, hostName, players, sets, pursePerTeam, timerDurationSeconds, reunsoldPhaseEnabled) {
    const teams = {};
    for (const t of types_1.IPL_TEAMS) {
        teams[t.code] = {
            ...t,
            ownerSessionId: null,
            ownerName: null,
